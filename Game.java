...
public class Game {
    public static double damageCalculator(Player[] players, int attacker, int receiver) {
        ...
        return ((((2 * 50 / 5 + 2) * players[attacker].movePower * atk / def) / 50 + 2) * random * 1.5 * type);
    } // end of damageCalculator

    public static void turn(Player[] players, int turn, File f) {
        ...
        if(players[randP1].Spe > players[randP2].Spe) { // 2nd is faster than 1st
            ...
            players[randP2].currentHP -= (int) damageCalculator(players, randP1, randP2);
